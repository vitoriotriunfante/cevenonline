...
- **Deploy publica da branch `clean-v3`** (confirmado 23/09/2026: `functions/api/version.js` da `clean-v3` = `v2.17.0-blocked-orders-6x-daily-schedule`, igual ao no ar; a `main` está em `v2.11.9`, defasada). Logo, WhatsApp e TV compartilham a mesma branch de produção — por isso o trabalho de TV vai na branch `cftv`.
