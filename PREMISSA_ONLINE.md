...
| 2 | **Lista de vendedores da TV** vem da cópia LOCAL de `VENDEDORES AUDITADOS.xlsx` gerada na hora de publicar | `gerar_mostra_tv.js` -> `public/mostra_vendedores.json` | Function do Cloudflare que lê a planilha direto do Google Drive (service account em secret) — `/api/tv-mostra` |
